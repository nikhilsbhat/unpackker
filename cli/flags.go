...
func registerFlags(cmd *cobra.Command) {
	cmd.PersistentFlags().StringVarP(&unpcker.Name, "name", "n", "demo", "name of the asset that has to be created")
	cmd.PersistentFlags().StringVarP(&unpcker.AssetPath, "asset", "a", "", "path to asset which needs to be packed")
	cmd.PersistentFlags().StringVarP(&unpcker.Environment, "dev", "e", "development", "name of environment in which the asset is packed")
